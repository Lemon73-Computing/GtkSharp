...
MANIFEST_BASE_NAME="mauigtk.net.sdk.gtk.manifest"
...
function install_gtkworkload() {
    ...
    mkdir -p $SDK_MANIFESTS_DIR/mauigtk.net.sdk.gtk
    cp -f $TMPDIR/unzipped/data/* $SDK_MANIFESTS_DIR/mauigtk.net.sdk.gtk/

    if [ ! -f $SDK_MANIFESTS_DIR/mauigtk.net.sdk.gtk/WorkloadManifest.json ]; then
    ...
    $DOTNET_INSTALL_DIR/dotnet workload install gtk --skip-manifest-update
    ...
    echo "Done installing Gtk workload $MANIFEST_VERSION"
    echo ""
}
...
if [ -z "$INSTALLED_DOTNET_SDKS" ]; then
    echo ".NET SDK version 6 or later is required to install Gtk Workload."
else
    for DOTNET_SDK in $INSTALLED_DOTNET_SDKS; do
        echo "Check Gtk Workload for sdk $DOTNET_SDK."
        install_gtkworkload $DOTNET_SDK
    done
fi
