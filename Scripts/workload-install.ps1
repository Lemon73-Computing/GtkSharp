#
# Copyright (c) Samsung Electronics, MauiGtk Community. All rights reserved.
# Licensed under the MIT license. See LICENSE file in the project root for full license information.
#

<#
.SYNOPSIS
Installs Gtk workload manifest.
.DESCRIPTION
Installs the WorkloadManifest.json and WorkloadManifest.targets files for Gtk to the dotnet sdk.
.PARAMETER Version
...
$ManifestBaseName = "MauiGtk.NET.Sdk.Gtk.Manifest"
$global:FallbackId = ""
...
function Install-Pack([string]$Id, [string]$Version, [string]$Kind) {
    ...
    switch ($Kind) {
        "manifest" {
            ...
            New-Item -Path $GtkManifestDir -ItemType "directory" -Force | Out-Null
            Copy-Item -Path "$TempUnzipDir\data\*" -Destination $GtkManifestDir -Force
        }
...
function Remove-Pack([string]$Id, [string]$Version, [string]$Kind) {
    switch ($Kind) {
        "manifest" {
            Remove-Item -Path $GtkManifestDir -Recurse -Force
        }
        {($_ -eq "sdk") -or ($_ -eq "framework")} {
            $TargetDirectory = $(Join-Path -Path $DotnetInstallDir -ChildPath "packs\$Id\$Version")
...
function Install-GtkWorkload([string]$DotnetVersion)
{
    $VersionSplitSymbol = '.'
    ...
    $GtkManifestDir = Join-Path -Path $ManifestDir -ChildPath "mauigtk.net.sdk.gtk"
    $GtkManifestFile = Join-Path -Path $GtkManifestDir -ChildPath "WorkloadManifest.json"

    # Check and remove already installed old version.
    if (Test-Path $GtkManifestFile) {
        $ManifestJson = $(Get-Content $GtkManifestFile | ConvertFrom-Json)
        $OldVersion = $ManifestJson.version
        if ($OldVersion -eq $Version) {
            $DotnetWorkloadList = Invoke-Expression "& '$DotnetCommand' workload list | Select-String -Pattern '^gtk'"
    ...
    $NewManifestJson = $(Get-Content $GtkManifestFile | ConvertFrom-Json)
    ...
    # Add gtk to the installed workload metadata.
    ...
    New-Item -Path $(Join-Path -Path $DotnetInstallDir -ChildPath "metadata\workloads\$DotnetVersionBand\InstalledWorkloads\gtk") -Force | Out-Null
    if (Test-Path $(Join-Path -Path $DotnetInstallDir -ChildPath "metadata\workloads\$DotnetVersionBand\InstallerType\msi")) {
        New-Item -Path "HKLM:\SOFTWARE\Microsoft\dotnet\InstalledWorkloads\Standalone\x64\$DotnetTargetVersionBand\gtk" -Force | Out-Null
    }

    # Clean up
    Remove-Item -Path $TempDir -Force -Recurse

    Write-Host "Done installing Gtk workload $Version"
}
...
{
    Write-Host "`n.NET SDK version 6 or later is required to install Gtk Workload."
}
else
{
    ...
    {
        try {
            Write-Host "`nCheck Gtk Workload for sdk $DotnetSdk"
            Install-GtkWorkload -DotnetVersion $DotnetSdk
        }
        catch {
            Write-Host "Failed to install Gtk Workload for sdk $DotnetSdk"
            Write-Host "$_"
            Continue
        }
    }
